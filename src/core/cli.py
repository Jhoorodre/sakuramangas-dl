...
def download_chapters():
    ...
    if escolha.isdigit() and 1 <= int(escolha) <= len(toml_links):
        ...
        idx = input("\nDigite o número correspondente (ou Enter para cancelar): ").strip()
        ...
        try:
                ...
                if "chapters" in old_data:
                    for cid, cdata in chapters_dict.items():
                        if cid in old_data["chapters"]:
                            cdata["downloaded"] = old_data["chapters"][cid].get("downloaded", False)
                            
                    manga_data["latest_downloaded_chapter"] = old_data.get("latest_downloaded_chapter", 0.0)
            ...
            for cap_id, cap in reversed(list(chapters_dict.items())):
                ...
                if selected:
                    ...
                    chapter_folder = re.sub(r'Cap\.\s*([\d\.]+)', lambda m: f"Cap {float(m.group(1)):06.2f}".replace(".00", "").replace(".", ",").zfill(3), raw_name)
                    # Fallback mais simples para o regex de nome de pasta
                    chapter_folder = re.sub(r'Cap\.\s*([\d\.]+)', lambda m: f"Cap {m.group(1).zfill(3)}", raw_name)
                    chapter_folder = re.sub(r'[\\/:*?"<>|]', "", chapter_folder)
                    chapter_path = os.path.join("download", manga_folder, chapter_folder)
                    
                    # Verificação Bilateral Interativa (Opção 1)
                    is_physically_downloaded = False
                    if os.path.exists(chapter_path):
                        if any(f.startswith("pag_") and f.endswith(".jpg") for f in os.listdir(chapter_path)):
                    ...
                    if is_physically_downloaded:
                        if overwrite_all:
                            ...
                            ans = input("Deseja SOBRESCREVER? (S=Sim / N=Não / T=Sim para Todos): ").strip().lower()
                            if ans == 't':
                                overwrite_all = True
                                ans = 's'
                                
                        if ans != 's':
                            print(f"  -> [PULO] Mantendo o capítulo original.")
                            cap["downloaded"] = True
                            do_download = False
                        else:
                            import shutil
                            shutil.rmtree(chapter_path, ignore_errors=True)
                            print(f"  -> Apagando versão antiga...")
                    else:
                        if cap.get("downloaded"):
                            print(f"\n[?] O {chapter_folder} consta no JSON como BAIXADO, mas sumiu da pasta (foi movido?).")
                            ans = input("Deseja baixar novamente para esta pasta? (S/N): ").strip().lower()
                            if ans != 's':
                                print(f"  -> [PULO] Ignorando re-download. Mantendo flag True no JSON.")
                                cap["downloaded"] = True
                                do_download = False
                                
                    if do_download:
                        print(f"  -> Baixando: {chapter_folder} na pasta {manga_folder}")
                        success = scraper.download_chapter(url, manga_name=manga_folder, chapter_name=chapter_folder)
                        if success:
                            cap["downloaded"] = True
    ...
    for link in links:
        ...
        try:
            ...
            if manga_data and "chapters" in manga_data:
                ...
                print(f"[+] Lista salva em {json_path}! Encontrados {len(manga_data['chapters'])} capítulos.")
                    ...
                    with open(json_path, 'r', encoding='utf-8') as f:
                        ...
                        for cap_id, cap in reversed(list(chapters_dict.items())):
                            ...
                            if url:
                                ...
                                if is_physically_downloaded:
                                    print(f"  -> [PULO] Capítulo já existente no HD: {chapter_folder}")
                                    cap["downloaded"] = True
                                else:
                                    if cap.get("downloaded"):
                                        # Usuário moveu para outro lugar para economizar espaço
                                        print(f"  -> [MEMÓRIA] {chapter_folder} sumiu do HD, mas JSON diz que já foi baixado. Presumindo movimentação. Pulando...")
                                    else:
                                        print(f"  -> Baixando Inédito: {chapter_folder} na pasta {manga_folder}")
                                        success = scraper.download_chapter(url, manga_name=manga_folder, chapter_name=chapter_folder)
                                        if success:
                                            cap["downloaded"] = True
