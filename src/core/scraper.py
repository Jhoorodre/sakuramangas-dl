...
from src.core.utils import handle_cloudflare, activate_scroll_mode, expand_chapters_list, JS_EXTRACT_MANGA, format_state_file, is_valid_image
...
class SakuraScraper:
    ...
    def download_chapter(self, url, manga_name=None, chapter_name=None, output_base_dir="download"):
        ...
        arquivos_prontos = [f for f in os.listdir(output_dir) if f.startswith("pag_") and f.endswith(".jpg")]
        if len(arquivos_prontos) > 0:
            logging.info(f"[PIPELINE] PULO INTELIGENTE: Capítulo já está 100% baixado ({len(arquivos_prontos)} páginas). Ignorando: {chapter_name}")
            return True
        ...
        with sync_playwright() as p:
            ...
            def handle_response(response):
                if "/imagens/" in response.url and response.url.endswith(".jpg"):
                    ...
                    for key, value in response.request.headers.items():
                        k = key.lower()
                        if k.startswith("x-") and k not in ["x-requested-with", "x-forwarded-for"]:
                    ...
                    try:
                        ...
                        if not is_valid_image(body):
                            logging.warning(f"  -> [!] Imagem {temp_filename} interceptada como HTML/Vazia. Passando para o Pós-Processamento...")
                        ...
                        if is_valid_image(body):
                            ...
                            logging.error(f"  -> [-] Imagem {temp_filename} irremediavelmente corrompida pela CDN.")
                    except Exception as e:
                        logging.error(f"  -> [-] Falha ao salvar {temp_filename}: {e}")

            page.on("response", handle_response)

            logging.info(f"[PIPELINE] Acessando SakuraMangas...")
            try:
            ...
            logging.info("[PIPELINE] Deixando a poeira baixar para as imagens iniciais carregarem em paz (10s)...")
            page.wait_for_timeout(10000)
            
            # Ativa o modo cascata (Scroll)
            activate_scroll_mode(page)

            logging.info("[PIPELINE] Acionando Scroll Brutal e Next Page para provocar Lazy Load...")
            timeout = 60
            last_len = -1
            stable_count = 0
            ...
            while timeout > 0:
                ...
                if random.random() < 0.10:
                    logging.info("  -> [STEALTH] Simulando leitura atenta (pausa dramática na página)...")
                    page.wait_for_timeout(random.randint(4000, 8000))
                    
                # HUMANO 2: O Scroll Desastrado (15% de chance)
                if random.random() < 0.15:
                    logging.info("  -> [STEALTH] Rolou muito rápido! Subindo a tela para reler...")
                ...
                if random.random() < 0.25:
                    page.mouse.move(random.randint(50, 1000), random.randint(50, 800), steps=random.randint(8, 25))
                ...
                if current_len > 0:
                    if current_len == last_len:
                        ...
                        if stable_count >= max_wait:
                            logging.info(f"[PIPELINE] Rodapé atingido / Sem novas imagens. Total de páginas: {current_len}")
                            break
                    else:
            ...
            for u in ordered_urls:
                if u not in downloaded_files:
                    logging.warning(f"  -> [!] Imagem perdida/corrompida detectada. Resfriando conexão e resgatando via JS Blob: {u}")
                    try:
                        ...
                        if b64 and "base64," in b64:
                            ...
                            if is_valid_image(body):
                                ...
                                logging.info(f"  -> [+] Imagem resgatada com sucesso direto da Memória do Navegador!")
                            else:
                                logging.error(f"  -> [-] Imagem inacessível no momento (Rate Limit Persistente).")
                    except Exception as e:
            ...
            if len(faltantes) > 0:
                ...
                try:
                    ...
                    logging.info("[PIPELINE] F5 Concluído. Deixando a poeira baixar novamente (10s)...")
                    page.wait_for_timeout(10000)
                    ...
                    faltantes_final = [u for u in ordered_urls if u not in downloaded_files]
                    if len(faltantes_final) > 0:
                        logging.error(f"[PIPELINE] Fim da linha. {len(faltantes_final)} imagens não puderam ser salvas.")
                    else:
                        logging.info("[PIPELINE] F5 Extremo foi um sucesso! Todas as imagens recuperadas.")
                except Exception as e:
                    logging.error(f"[PIPELINE] Erro catastrófico no F5: {e}")
            
            # Renomeação Ordenada (pag_001.jpg, pag_002.jpg)
            logging.info("[PIPELINE] Formatando e numerando as imagens no disco (pag_001.jpg)...")
